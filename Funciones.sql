...
AS BEGIN
	...
	SET @saldo = ( SELECT SUM(M.ImporteMovim)
					FROM Movimiento M
					WHERE M.IdCuenta = @cuenta
					AND M.TipoMovim = 'E'
...
AS BEGIN
	...
	SET @saldo = (SELECT SUM(Mov.ImporteMovim) 
					FROM Movimiento Mov
	...
	BEGIN
		...
		IF EXISTS( SELECT 1 FROM Cliente Cli 
			...
			BEGIN
				...
				IF (@sobregiro > 0)
					BEGIN
						SET @salida = 'Cliente: ' + (SELECT NombreCliente FROM Cliente WHERE IdCliente = @id_cliente) + ', NO POSEE SOBREGIRO AL DÍA DE HOY.';
					END
				ELSE
					--El sobregiro solo existe cuando la resta de las entradas y las salidas da números negativos.
					BEGIN
						SET @salida = 'Cliente: ' + (SELECT NombreCliente FROM Cliente WHERE IdCliente = @id_cliente) + ', Sobregiro actual: '+@sobregiro+' USD';
					END
			END
